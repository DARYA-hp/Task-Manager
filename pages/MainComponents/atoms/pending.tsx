...
export default function Pending({ projectName, filters }: PendingProps) {
    ...
    let projectTasks = tasks.filter(t => t.projectName === projectName && t.status === 'pending');
    ...
    console.log(`Pending Column - Tasks Count: ${projectTasks.length}`, projectTasks);

    return (
        <>
            <div id="pending" className=" flex flex-col gap-y-4 w-[240px]" >
            ...
            <Modal isOpen={newTask} onClose={() => setNewTask(false)} title="">
                <NewTask projectName={projectName} onClose={() => setNewTask(false)} defaultStatus="pending" />
            </Modal>
        </>
    );
}