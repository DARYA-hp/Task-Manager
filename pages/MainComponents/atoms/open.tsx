...
export default function Open({ projectName, filters }: OpenProps) {
    ...
    let projectTasks = tasks.filter(t => t.projectName === projectName && t.status === 'open');
    ...
    console.log(`Open Column - Tasks Count: ${projectTasks.length}`, projectTasks);

    return (
        <>
            <div id="open" className=" flex flex-col gap-y-4 w-[240px]" >
            ...
            <Modal isOpen={newTask} onClose={() => setNewTask(false)} title="">
                <NewTask projectName={projectName} onClose={() => setNewTask(false)} defaultStatus="open" />
            </Modal>
        </>
    );
}